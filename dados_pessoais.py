...
import requests
import numpy
from utils import gerador, CNPJ, data_nascimento
...
for i, nome in enumerate(ns):
    ...
    sexo = 'Masculino'
    cursor.execute(f"""INSERT INTO dados(Nome, Sobrenome, Sexo, CPF, CNPJ, CEP, UF, Cidade,  Bairro, Rua, Nascimento) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                   (str(nome), str(sb[i]), str(sexo), gerador(True), CNPJ().gera(), numpy.random.choice(ceps), estado, cidade, bairro, rua, data_nascimento())),
    conexao.commit()
